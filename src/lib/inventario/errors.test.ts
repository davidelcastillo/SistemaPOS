...
function p2002Adapter(fields: string[]): Prisma.PrismaClientKnownRequestError {
  return new Prisma.PrismaClientKnownRequestError("Unique constraint failed", {
    code: "P2002",
    clientVersion: "test",
    meta: {
...
describe("mapPrismaError (CM-R1 / PM-R2)", () => {
  ...
  it("maps a P2002 with driver-adapter meta on sku to DUPLICATE_SKU", () => {
    const result = mapPrismaError(p2002Adapter(["sku"]));
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.code).toBe("DUPLICATE_SKU");
    }
  ...
  it("maps a P2002 with driver-adapter meta on name to DUPLICATE_CATEGORY", () => {
    const result = mapPrismaError(p2002Adapter(["name"]));
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.code).toBe("DUPLICATE_CATEGORY");
    }
