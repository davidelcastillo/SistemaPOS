...
| prisma-* | Prisma 7 setup, client API, CLI, driver adapters, upgrade guides | `.agents/skills/prisma-*/` |
...
| --- | --- |
| `DATABASE_URL` | URL de conexión a PostgreSQL (driver adapter Prisma 7). Ej: `postgresql://postgres:admin123@localhost:5433/ssr-sistema-tickets?schema=public` |
| `AUTH_URL` | URL canónica de la app (para NextAuth) |
| `AUTH_SECRET` | Clave para firmar/encriptar tokens y sesiones (NextAuth) |
| `APP_ENV` | `development`, `staging` o `production` |

> ⚠️ **Prisma 7**: la URL de conexión se lee desde `prisma7.config.ts` (`import "dotenv/config"`), NO desde `schema.prisma`. El datasource en el schema no lleva `url` ni `directUrl`.
...
1. **Definición del Stack:** Next.js + TypeScript + Prisma 7 + PostgreSQL (ya definido; ver sección 5).
...
4. **Contrato de Datos Tipado:** schemas **Zod** compartidos (`src/lib/`) y tipos TypeScript derivados. El contrato es un tipo que el compilador verifica, no un JSON informal.
...
5. **Usar Context7** para documentación de librerías antes de escribir código que involucre Next.js, Prisma, etc.
6. **Aplicar buenas prácticas del stack:** Server Actions para mutaciones, RSC para UI estática, Route Handlers para lógica API.
7. **Seguridad y autenticación:** usuarios en tabla `User`, contraseñas con `bcrypt`, sesión con NextAuth (JWT), rutas internas protegidas por middleware validando token y rol.
8. **Testing:** unit tests (`npm test` = `vitest run`), integración (Prisma + PostgreSQL), E2E (`npm run test:e2e` = `playwright test`). Ejecutar `npm test` antes de cada `sdd-apply` y E2E antes de `sdd-verify`.
9. **Documentación:** cada feature con su archivo en `docs/`; actualizar `README.md` con resumen y enlaces.
10. **Versionado y releases:** Conventional Commits. En `main`, validar receipt de revisión y que no haya cambios pendientes.

---

## 📚 5. Enlaces a documentación crítica (actualizados vía Context7)

- **Next.js** – https://nextjs.org/docs (App Router, RSC, Server Actions). Ver también `node_modules/next/dist/docs/` (documentación local de esta versión).
...
- [ ] Verificar que el servicio de base de datos esté levantado (`docker ps` o `pg_isready`).
...
- [ ] Si la tarea implica cambios de esquema, asegurarse de que `npx prisma migrate dev` pueda ejecutarse.
...
- [ ] Al finalizar, verificar el **Definition of Done** (sección 3.3) y archivar con `sdd-archive`.

---

## 👥 7. Trabajo colaborativo (equipo)
...
2. **Contrato de datos:** los schemas Zod de `src/lib/<modulo>/schemas.ts` los modifica el dueño del módulo; los schemas realmente compartidos van a `src/lib/` compartido y se modifican SOLO con coordinación del equipo.
3. **Lógica compartida:** helpers y formateo van a `src/lib/` compartido; crear o modificar esos archivos SOLO con coordinación.
4. **Base de datos:** `prisma/schema.prisma` y sus migraciones los administra un único dueño a la vez; nunca editar una migración ya aplicada — crear una nueva. Con Prisma 7 el output del cliente es `src/generated/prisma` (no tocar a mano).
5. **Archivos calientes** (`src/app/layout.tsx`, `src/middleware.ts`, `src/lib/db.ts`, `src/lib/auth.ts`, `package.json`): solo se modifican con acuerdo del equipo o por su dueño designado.
...
8. Al instalar dependencias nuevas, avisar al compañero (cambian `package.json` / lockfile).
